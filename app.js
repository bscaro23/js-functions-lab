...
const isCharAVowel = (char) => {
    if (char.length !== 1) {
        return 'Input must be one character';
    }
...
const createUsername = (first, last) => {
    ...
    if (fl < 3 || ll < 3) {
        return 'Invalid length of First or Last name';
    }
    return first.substring(0, 3) + last.substring(0, 3) + (fl + ll);
}
